...
from brepprediff.data.graph import BRepGraph, collate_graphs, graph_from_arrays, normalize_graph_features
...
def _occ_safe_step_path(path: Path) -> Iterator[Path]:
    ...
    if not _path_needs_ascii_staging(path):
        yield path
        return
    with tempfile.TemporaryDirectory(prefix="brepprediff_step_") as tmp_dir:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--checkpoint", required=True, help="Finetuned BRepPreDiff checkpoint.")
