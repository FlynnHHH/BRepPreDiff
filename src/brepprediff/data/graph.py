...
def rotate_graph_geometry(
    graph: BRepGraph,
    *,
    uv_grid_size: int,
    edge_u_grid_size: int,
) -> BRepGraph:
    """Apply one random label-preserving SO(3) rotation to raw CAD geometry."""
    face = graph.face_cont.clone()
    edge = graph.edge_cont.clone()

    quaternion = torch.nn.functional.normalize(
        torch.randn(4, dtype=face.dtype, device=face.device), dim=0
    )
    ...
    face_grid = face[:, 11:].reshape(-1, uv_grid_size, uv_grid_size, 7)
    face_grid[..., :3] = face_grid[..., :3] @ rotation.T
    face_grid[..., 3:6] = face_grid[..., 3:6] @ rotation.T
    if edge.shape[0]:
        edge_grid = edge[:, 3:].reshape(-1, edge_u_grid_size, 6)
        edge_grid[..., :3] = edge_grid[..., :3] @ rotation.T
        edge_grid[..., 3:6] = edge_grid[..., 3:6] @ rotation.T
