...
def extract_mfcad_face_labels(step_path: str | Path) -> list[int]:
    """Read labels in the exact OCC face order consumed by Blendit.
    ...
    from blendit.brep.occ_extractor import _occ_imports
...
def write_blendit_split_files(
    ...
    *,
    ...
    """Write split entries relative to the shared STEP root used by Blendit."""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--blendit-splits-dir",
        help="Optional output directory for Blendit-compatible prefixed split files.",
    )
    parser.add_argument(
        "--splits",
        nargs="+",
    ...
    if args.blendit_splits_dir:
        outputs = write_blendit_split_files(
            args.dataset_root,
            args.step_root,
            args.blendit_splits_dir,
            splits=args.splits,
        )
        for split, output in outputs.items():
            print(f"Blendit split {split}: {output}")
