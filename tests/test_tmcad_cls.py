...
def test_prepare_tmcad_classification_generates_one_hot_and_stratified_splits(tmp_path):
    from brepprediff.data.tmcad_cls import prepare_tmcad_classification
    ...
    class_map = json.loads((root / "brepprediff_splits" / "class_map.json").read_text())
    ...
    for split, expected_per_class in (("train", 6), ("val", 2), ("test", 2)):
        items = (root / "brepprediff_splits" / f"{split}.txt").read_text().splitlines()
