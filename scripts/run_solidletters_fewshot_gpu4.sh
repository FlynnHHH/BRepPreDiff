#!/usr/bin/env bash
# Add SolidLetters 10/20-shot evaluations to the existing few-shot result set.
set -euo pipefail
...
CONFIG="$ROOT_DIR/configs/finetune_solidletters_mlp.yaml"
...
run_shot() {
  ...
  local run_name="solidletters_cls_${shot}shot_pre50_ft100_seed42_${RUN_TAG}"
  local result_dir="$RESULT_ROOT/solidletters_cls/${shot}shot"
  local task_log="$LOG_ROOT/solidletters_cls_${shot}shot.log"
  local run_dir

  mkdir -p "$result_dir"
  cp "${split%.txt}.audit.json" "$result_dir/selection.audit.json"
  echo "[$(date --iso-8601=seconds)] train task=solidletters_cls shot=$shot gpu=$GPU_ID"
  ...
  if [[ -z "$run_dir" ]] || [[ ! -f "$run_dir/checkpoints/best.pt" ]]; then
    echo "Best checkpoint not found for SolidLetters ${shot}-shot" >&2
    return 1
  fi
  echo "[$(date --iso-8601=seconds)] evaluate task=solidletters_cls shot=$shot"
...
if [[ "$status" -ne 0 ]]; then
  echo "SolidLetters few-shot run failed; inspect $LOG_ROOT" >&2
  exit "$status"
fi
...
echo "[$(date --iso-8601=seconds)] SolidLetters few-shot complete report=$REPORT_PATH"
