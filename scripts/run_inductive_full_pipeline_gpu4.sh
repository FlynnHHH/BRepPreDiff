#!/usr/bin/env bash
# Automated GPU-4 pipeline: full-loss inductive pretraining followed by
# bounded-parallel full-data MLP and aligned-DiffLoss fine-tuning/evaluation.
set -euo pipefail
...
PRETRAIN_LR_SCHEDULER="${PRETRAIN_LR_SCHEDULER:-cosine}"
...
MAX_PARALLEL_FINETUNES="${MAX_PARALLEL_FINETUNES:-3}"
RUN_TAG="${RUN_TAG:-inductive9_no_fab_full_${PRETRAIN_LR_SCHEDULER}_pre${PRETRAIN_EPOCHS}_ft${FINETUNE_EPOCHS}_seed42_$(date +%Y%m%d-%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/inductive_full}"
LOG_ROOT="${LOG_ROOT:-$ROOT_DIR/runs/launch_logs/$RUN_TAG}"
REPORT_PATH="${REPORT_PATH:-$ROOT_DIR/reports/${RUN_TAG}.md}"
PRETRAIN_NAME="inductive9_no_fab_full_lr${PRETRAIN_LR_TAG}_${PRETRAIN_LR_SCHEDULER}_e${PRETRAIN_EPOCHS}_seed42_$RUN_TAG"
PRETRAIN_CHECKPOINT_OVERRIDE="${PRETRAIN_CHECKPOINT_OVERRIDE:-}"
PRETRAIN_RESUME_BASE="${PRETRAIN_RESUME_BASE:-}"
PRETRAIN_DESCRIPTION="${PRETRAIN_DESCRIPTION:-${PRETRAIN_EPOCHS}-epoch full-loss inductive 9-source（无 FabWave）encoder；LR=${PRETRAIN_LR}、scheduler=${PRETRAIN_LR_SCHEDULER}、min_lr=${PRETRAIN_MIN_LR}}"
  ...
  if [[ -n "$pretrain_run" && -f "$pretrain_run/checkpoints/last.pt" ]] \
    ...
    if [[ "$WANDB_ENABLED" == "true" ]]; then
      pretrain_args+=(
        ...
        --override "wandb.tags=[inductive,full_loss,pretrain${PRETRAIN_EPOCHS},no_fabwave,$PRETRAIN_LR_SCHEDULER,gpu4]"
      )
    fi
...
run_downstream() {
  ...
  if [[ "$WANDB_ENABLED" == "true" ]]; then
    args+=(
      ...
      --override "wandb.tags=[full_data,$head,finetune${FINETUNE_EPOCHS},gpu4]"
    )
  fi
...
conda run --no-capture-output -n blendit python scripts/compare_new_occ_all_heads.py \
  "${report_args[@]}" \
  --title "Inductive 9-source encoder：全量下游 MLP 与 DiffLoss 对照" \
  --epochs "$FINETUNE_EPOCHS" \
  --batch-description "task batch size 256/512、梯度累积 1、最多 $MAX_PARALLEL_FINETUNES 路并行" \
