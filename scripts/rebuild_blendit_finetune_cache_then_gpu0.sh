#!/usr/bin/env bash
# Rebuild the BlendIt downstream cache for the current OCC schema, then run the
...
LOG_DIR="$ROOT_DIR/runs/launch_logs/blendit_finetune_cache_$RUN_TAG"
...
echo "[$(date --iso-8601=seconds)] rebuilding BlendIt downstream cache with workers=$workers"
