# CARD 与 BRepPreDiff DiffLoss 分类头对比调研
...
- BRepPreDiff 当前仓库中的 DiffLoss 分类头、标签扩散公共实现及主实验配置。

## 2. 核心结论

BRepPreDiff DiffLoss 与 CARD 属于同一条技术路线：二者都把离散类别映射成连续的
类别原型，在标签空间加入高斯噪声，并以输入特征为条件恢复类别原型。

但 BRepPreDiff 不是 CARD 的直接复现。当前 BRepPreDiff 已经去掉 CARD 最有辨识度的
“预训练分类器条件均值”，并把主配置调整为 `x_start + 1-step DDIM + 单样本推理`。
因此，更准确的定位是：

> BRepPreDiff DiffLoss 是一个 CARD-inspired conditional label-diffusion head，
> 即通过扩散噪声训练的条件类别原型预测头。

CARD 的重点是学习完整的条件分布 $p(y\mid x)$ 和进行实例级不确定性评估；
BRepPreDiff 当前实现的重点则是利用标签噪声、时间条件和类别原型几何改进下游分类，
在实际推理形态上更接近判别式分类头。
...
## 4. BRepPreDiff 当前 DiffLoss 分类头

### 4.1 B-Rep 图条件

BRepPreDiff 先使用 B-Rep 图编码器产生各个面的 embedding，再对一个 CAD 模型中的所有面
进行均值池化：
...
- `src/brepprediff/models/classification.py::mean_graph_pool`
- `src/brepprediff/models/classification.py::DiffusionClassificationModel`
...
BRepPreDiff 使用双极 one-hot，而不是 CARD 的普通 one-hot：
...
- `src/brepprediff/models/diffusion.py::bipolar_one_hot`

### 4.3 标准零中心前向扩散

BRepPreDiff 的标签前向过程为：
...
- `src/brepprediff/models/diffusion.py::LabelDiffusionSchedule.q_sample`
...
所以 BRepPreDiff 扩散先验仍然以零为中心，图输入仅通过去噪器的 condition 分支进入模型。

### 4.4 没有 CARD 式基础分类器

BRepPreDiff 的去噪器可以表示为：
...
BRepPreDiff 的预训练 checkpoint 对分类任务只加载 `encoder.*`，不会把预训练任务 head
映射到图级分类头。因此：

- CARD 预训练分类器提供类别概率先验；
- BRepPreDiff 预训练模型只提供几何表征初始化；
- BRepPreDiff 的 encoder 和 DiffLoss head 在下游训练中端到端微调。

相关实现：

- `src/brepprediff/training/common.py::load_pretrain_checkpoint_for_finetune`
- `src/brepprediff/models/downstream.py::LabelDiffusionModel`
...
- `src/brepprediff/models/downstream.py::prepare_label_diffusion_training_batch`
- `src/brepprediff/models/downstream.py::label_diffusion_objective`
...
当前主配置已经改成纯 `x_start`、1-step DDIM。因此分析“现在的 BRepPreDiff DiffLoss”时，
...
若 `inference_samples` 大于 1，BRepPreDiff 会平均不同样本的 softmax 概率；当前主配置为 1，
所以没有进行 Monte Carlo 分布估计。

相关实现：

- `src/brepprediff/models/downstream.py::stable_initial_noise`
- `src/brepprediff/models/downstream.py::predict_label_diffusion_probabilities`
- `src/brepprediff/models/diffusion.py::LabelDiffusionSchedule.sample`

## 5. CARD 与 BRepPreDiff 逐项对比

| 项目 | CARD | BRepPreDiff 当前 DiffLoss |
|---|---|---|
...
BRepPreDiff 继承了 CARD 或同类 label-space diffusion 方法的三项基本思想：

1. 将分类视为连续标签空间中的条件生成或恢复问题；
2. 对类别原型进行 Gaussian diffusion/noising；
3. 使用 MSE 去噪目标代替必须直接优化交叉熵。

但是，BRepPreDiff 没有继承 CARD 最关键的完整机制：
...
CARD 使用 $\{0,1\}^C$ 原型，并根据到正值 1 的平方距离计算类别分数。BRepPreDiff 使用
$\{-1,+1\}^C$ 原型，直接把重构值作为 softmax score。

BRepPreDiff 推理会把 $\hat z_0$ 裁剪到 $[-1,1]$。在这个区间内：
...
BRepPreDiff 直接使用 $z_c$ 通常会产生相同的 top-1 类别。

不过，两者的 score 非线性不同，所以 softmax 概率、置信度和校准结果并不相同。
这意味着标签编码差异对 argmax 的影响较小，但对概率解释有明显影响。

## 8. 如何理解当前 BRepPreDiff DiffLoss 的收益

当前 BRepPreDiff DiffLoss 的主要效果更可能来自以下组合，而不是 CARD 所强调的完整
条件分布建模：
...
在项目文档或论文中，不建议直接写“BRepPreDiff 使用 CARD 分类器”，因为两者在条件先验、
训练目标和采样协议上都有实质差异。
...
> BRepPreDiff 使用受 CARD 启发的标签空间扩散分类头。类别被编码为双极 one-hot 原型，
...
BRepPreDiff 当前 DiffLoss 是以 B-Rep graph embedding 为条件的标签原型恢复模型：它使用
双极 one-hot、零中心高斯扩散、纯 `x_start` MSE，以及单样本单步 DDIM 推理。两者具有
相同的标签空间扩散思想，但在条件先验、损失、采样方式和实际目标上已经明显分化。

一句话概括：

> CARD 学习“给定输入可能产生怎样的标签分布”；BRepPreDiff 当前 DiffLoss 学习“给定
...
1. `src/brepprediff/models/classification.py`
2. `src/brepprediff/models/downstream.py`
3. `src/brepprediff/models/diffusion.py`
4. `src/brepprediff/training/common.py`
