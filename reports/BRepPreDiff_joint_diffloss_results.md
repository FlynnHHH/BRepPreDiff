# BRepPreDiff 联合预训练基模 DiffLoss 调参结果

生成日期：2026-07-24

## 结论

使用联合自监督预训练基模，在同一 BRepPreDiff
...
- BRepPreDiff 数据：6,127 train / 766 validation / 766 test；
- 测试集：41,459 faces；
- MLP 对照：
  `runs/finetune/20260723-232008_joint_brepprediff_mlp/checkpoints/best.pt`，
  best epoch 21；
...
最终推荐配置为
`configs/finetune_joint_brepprediff_diffloss.yaml`。已验证 checkpoint 来自：

`runs/finetune/20260724-003104_joint_brepprediff_diff_s2_xstart_mildcw/checkpoints/best.pt`
...
- 新增联合基模 BRepPreDiff DiffLoss 配置；
...
`runs/finetune/20260724-003104_joint_brepprediff_diff_s2_xstart_mildcw/test_tuned_inference.json`
...
CUDA_VISIBLE_DEVICES=0 /home/hhfeng/miniconda3/envs/brepprediff/bin/python \
  ...
  --checkpoint runs/finetune/20260724-003104_joint_brepprediff_diff_s2_xstart_mildcw/checkpoints/best.pt \
  --split test \
  --output runs/finetune/20260724-003104_joint_brepprediff_diff_s2_xstart_mildcw/test_tuned_inference.json \
  --batch-size 512 \
  --num-workers 8 \
  --device cuda \
